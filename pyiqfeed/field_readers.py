...
def read_is_short_restricted(field: str) -> bool:
    ...
    if field != "":
        ...
        if field == "N":
            ...
            raise UnexpectedField("Unknown Value in Short Restricted Field: %s" % field)
    else:
        return False
...
def read_tick_direction(field: str) -> np.int8:
    ...
    if field != "":
        ...
        if field_as_int == 183:
            ...
            raise UnexpectedField("Unknown value in Tick Direction Field: %s" % field)
    else:
        return np.int8(0)
...
def read_hhmmssmil(field: str) -> int:
    ...
    if field != "":
        ...
        second = int(field[6:8])
        msecs = int(field[9:])
        return (1000000 * ((3600 * hour) + (60 * minute) + second)) + (1000 * msecs)
    else:
        return 0
...
def date_us_to_datetime(
    dt64: np.datetime64, tm_int: Union[int, np.datetime64]
) -> datetime.datetime:
